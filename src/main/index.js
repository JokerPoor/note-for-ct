...
function createWindow() {
  ...
  ipcMain.handle('gitee:test', async (_evt, { owner, repo, account } = {}) => {
    try {
      ...
      let token = null
      try {
        if (account && keytarLib) token = await keytarLib.getPassword(CREDENTIAL_SERVICE, account)
      } catch {}
      if (!token && account) token = tempCredentials.get(account) || null
...
app.whenReady().then(async () => {
  ...
  // 内存中的当前 Vault 路径与临时凭据存储（重启即失效，作为 keytar 不可用时的兜底）
  let currentVaultDir = ''
  const tempCredentials = new Map() // key: account, value: token
  const CREDENTIAL_SERVICE = 'CuteNotes:Gitee'
  let keytarLib = null
  try {
    // 动态加载，避免在未安装时崩溃
    keytarLib = (await import('keytar')).default
  } catch {}
  ...
  // 保存凭据（优先使用 keytar，失败则回落到内存）
  ipcMain.handle('credential:save', async (_evt, { account, token }) => {
    if (!account || !token) return { ok: false, reason: '账号或令牌为空' }
    try {
      if (keytarLib) {
        await keytarLib.setPassword(CREDENTIAL_SERVICE, account, token)
      } else {
        tempCredentials.set(account, token)
      }
      log.info('[凭据保存] 账号:', account, keytarLib ? '(keytar)' : '(内存)')
      return { ok: true }
    } catch (e) {
      // 回退到内存
      tempCredentials.set(account, token)
      log.warn('[凭据保存] keytar 写入失败，已回退到内存。账号:', account, '错误:', String(e?.message || e))
  ...
  ipcMain.handle('credential:get', async (_evt, { account }) => {
    ...
    try {
      if (keytarLib) {
        const token = await keytarLib.getPassword(CREDENTIAL_SERVICE, account)
        if (!token) return { ok: false, reason: '未找到凭据' }
        log.info('[凭据读取] 账号:', account, '(keytar)')
  ...
  ipcMain.handle('credential:clear', async (_evt, { account }) => {
    ...
    try {
      if (keytarLib) {
        await keytarLib.deletePassword(CREDENTIAL_SERVICE, account)
      }
    } catch {}
