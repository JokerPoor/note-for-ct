...
// 保存凭据（将配置信息写入持久化设置；若提供 PAT，则写入 keytar）
// 保存凭据与相关设置
const onSaveCredential = async () => {
  ...
  // 若提供 PAT，则保存到 keytar
  if (pat.value) {
    const res = await window.api.saveCredential(repoForm.giteeOwner, pat.value)
    if (!res?.ok) {
      ElMessage.error(`PAT 保存失败：${res?.reason || ''}`)
      await log.error('PAT 保存失败：', res?.reason || '')
    } else {
      patSaved.value = true
      // 如主进程回退到内存，提示用户需要重建原生依赖
      if (res.fallback) {
        ElMessage.warning(
          'keytar 未加载，当前 PAT 仅临时有效（重启即失效）。请执行 install-app-deps 并重启应用'
        )
        await log.warn('keytar 未加载，PAT 以临时方式保存（内存）')
      }
...
// 根据当前 Owner 检查是否已保存过 PAT（keytar）
// 检查当前 Owner 是否已保存 PAT（不回显值）
    ...
    <el-card shadow="never" class="mb-4">
      <template #header>
        <div class="flex items-center justify-between w-full">
          ...
          <span class="text-xs" :class="patSaved ? 'text-green-600' : 'text-gray-400'">
            {{ patSaved ? '已保存' : '未保存' }}
          </span>
        </div>
      </template>
